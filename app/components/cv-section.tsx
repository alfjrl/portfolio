...
function SectionLabel({
  ...
  return (
    ...
    >
      <h2 className="text-lg uppercase text-ink font-bold md:text-right">
        {children}
      </h2>
    </div>
  );
...
function WorkEntry({ entry, delay }: { entry: WorkExperience; delay: number }) {
  return (
    <div
      ...
      style={{ animationDelay: `${delay}s` }}
    >
      <div className="text-md text-muted tabular-nums flex-shrink-0 w-32 mb-2 sm:mb-0 leading-snug">
        {entry.years}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-md font-bold text-muted mb-2">
          {entry.company.en}
          {entry.company.zh && (
        ...
        {entry.description.length > 0 && (
          <div className="">
            {entry.description.map((para, i) => (
              <p key={i} className="text-md text-muted leading-relaxed">
                {para}
              </p>
            ))}
          </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-lg font-bold text-ink leading-snug mb-2">
          {entry.school.en}
          {entry.school.zh && (
            <span className="text-sm text-muted"> · {entry.school.zh}</span>
          )}
        </p>
  ...
  return (
    ...
    >
      ...
      <a
        ...
        className="text-md font-bold text-ink leading-snug underline underline-offset-2 decoration-line hover:decoration-muted transition-all"
      >
        {item.display}
      </a>
    </div>
