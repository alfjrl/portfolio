...
export default function ArticleCard() {
  ...
  const cards = articles.map((data) => (
    ...
    >
      <div className="absolute bottom-0 px-5 sm:px-8 w-[calc(100%-40px)] sm:w-[calc(100%-64px)] h-[250px] sm:h-[200px] bg-slate-700"></div>
      <Link
        href={data.link}
        target="_blank"
        rel="noopener noreferrer"
        className="relative w-full h-[250px] sm:h-[200px] border border-slate-700 bg-white transition-all duration-100 ease-in hover:border-slate-900 overflow-hidden group-hover:-translate-y-2 group-hover:-translate-x-2"
      >
